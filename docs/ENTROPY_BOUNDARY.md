...
| `services/api-service` publication software signer | Deterministic `derive_from_secret` from externally managed secret | Forbidden in production | Boot fails in production if software signer is selected or if no signer is configured; outside production the provider must be explicitly set to `software-ed25519` and the secret must be at least 32 bytes |
