...
fn should_expose_health_version(debug_build: bool) -> bool {
    let explicit_opt_in = std::env::var("HEALTH_EXPOSE_VERSION")
        .ok()
        .map(|v| matches!(v.to_ascii_lowercase().as_str(), "1" | "true" | "yes" | "on"))
        .unwrap_or(false);
    let production_env = ["ENV", "APP_ENV", "RUST_ENV"].iter().any(|k| {
        std::env::var(k)
            .ok()
    ...
    if production_env {
        // Fail-closed in production: version exposure is opt-in only.
        explicit_opt_in
    } else {
...
mod tests {
    ...
    fn test_health_check_redacts_version_by_default_in_production_mode() {
        ...
        let previous_env = std::env::var("ENV").ok();
        std::env::remove_var("HEALTH_EXPOSE_VERSION");
        std::env::set_var("ENV", "production");

        assert!(!should_expose_health_version(cfg!(debug_assertions)));
