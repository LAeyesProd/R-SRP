...
The fixture itself is generated by Rust. Regenerate and verify that it is clean with:

```powershell
cargo run --locked -p rsrp-proof-engine --example generate_proof_envelope_vectors
git diff --exit-code -- docs/PROOF_ENVELOPE_V1_TEST_VECTORS.json
```
