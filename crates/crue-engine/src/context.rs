...
impl EvaluationContext {
    ...
    pub fn from_request(request: &EvaluationRequest) -> Self {
        ...
        ctx.set_field(
            "agent.level",
            FieldValue::String(request.agent_level.clone()),
        );
        ctx.set_field(
            "agent.requests_last_hour",
            FieldValue::Number(request.requests_last_hour as i64),
        );
        ctx.set_field(
            "agent.requests_last_24h",
            FieldValue::Number(request.requests_last_24h as i64),
        );
        ...
        if let Some(justification) = &request.justification {
            ctx.set_field(
                ...
                FieldValue::String(justification.clone()),
            );
            ctx.set_field(
                "request.justification_length",
                FieldValue::Number(justification.len() as i64),
            );
        }
        if let Some(export_format) = &request.export_format {
            ctx.set_field(
                "request.export_format",
                FieldValue::String(export_format.clone()),
            );
        }
        ...
        // Results
        ctx.set_field(
            "request.results_last_query",
            FieldValue::Number(request.results_last_query as i64),
        );
        ...
        // Time
        ctx.set_field(
            "context.request_hour",
            FieldValue::Number(request.request_hour as i64),
        );
        ctx.set_field(
            "context.is_within_mission_hours",
            FieldValue::Boolean(request.is_within_mission_hours),
        );
...
impl FieldValue {
    ...
    pub fn compare(&self, op: &str, other: &FieldValue) -> bool {
        match (self, other) {
            ...
            (FieldValue::Boolean(a), FieldValue::Boolean(b)) => match op {
                "==" => a == b,
                "!=" => a != b,
                _ => false,
            },
...
mod tests {
    ...
    fn test_field_comparison() {
        ...
        assert!(FieldValue::String("test".to_string())
            .compare("==", &FieldValue::String("test".to_string())));
    }
}
